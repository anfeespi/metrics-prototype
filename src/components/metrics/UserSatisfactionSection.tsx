...
function ScoreGauge({ score }: { score: number }) {
  ...
  return (
    <div className={`yuno-card ${bg} p-6 flex flex-col items-center justify-center`}>
      <p className="text-sm text-yuno-muted mb-1">Satisfaction Score</p>
      <p className={`text-5xl font-bold ${color}`}>{score}</p>
      <p className="text-xs text-yuno-muted mt-1">out of 100</p>
    </div>
  );
}
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-4 gap-4 mb-4">
        ...
        <div className="yuno-card p-4 flex flex-col justify-center">
          <p className="text-sm text-yuno-muted mb-1">vs Previous Period</p>
          <p className={`text-lg font-semibold ${pctChange >= 0 ? 'text-accent-green' : 'text-accent-red'}`}>
            {pctChange >= 0 ? '↑' : '↓'} {Math.abs(pctChange).toFixed(1)}%
          </p>
          <p className="text-xs text-yuno-muted mt-1">from {data.prevScore}</p>
        </div>
        <div className="col-span-2 yuno-card p-4">
          <p className="text-sm text-yuno-muted mb-3">Sentiment Distribution</p>
          <ResponsiveContainer width="100%" height={80}>
            <BarChart data={data.distribution} layout="vertical" margin={{ left: 0, right: 0, top: 0, bottom: 0 }}>
              <XAxis type="number" hide />
              <YAxis type="category" dataKey="label" width={70} tick={{ fill: '#8e95a5', fontSize: 12 }} axisLine={false} tickLine={false} />
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="yuno-card p-4">
          <p className="text-sm text-yuno-muted mb-3">Score Trend</p>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={data.trend} margin={{ top: 4, right: 16, bottom: 4, left: 0 }}>
              ...
              <YAxis tick={{ fill: '#8e95a5', fontSize: 11 }} domain={[0, 100]} />
              <Tooltip contentStyle={TOOLTIP_STYLE} labelStyle={{ color: '#282a30' }} itemStyle={{ color: '#282a30' }} />
              <Line type="monotone" dataKey="score" stroke="#22c55e" strokeWidth={2} dot={{ fill: '#22c55e', r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        <div className="yuno-card p-4 overflow-x-auto">
          <p className="text-sm text-yuno-muted mb-3">Recent Conversations</p>
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.recentConversations.map((c, i) => (
                <tr key={i} className="border-b border-yuno-border/20">
                  <td className="py-2 pr-2 text-yuno-muted text-xs whitespace-nowrap">
                    {new Date(c.date).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  </td>
                  <td className="py-2 pr-2 text-yuno-text text-xs">{c.durationMin}m</td>
                  <td className="py-2 pr-2 text-yuno-text font-medium text-xs">{c.score}</td>
                  <td className={`py-2 pr-2 text-xs font-medium ${LABEL_COLORS[c.label]}`}>{c.label}</td>
                  <td className="py-2 text-yuno-muted text-xs truncate max-w-xs">{c.summary}</td>
                </tr>
              ))}
            </tbody>
          </table>
