...
export function TaskCompletionSection({ data }: { data: TaskCompletion }) {
  ...
  return (
    <section>
      ...
      <div className={`rounded-lg px-4 py-2 mb-4 text-sm flex items-center gap-2 ${isStale ? 'bg-accent-yellow/10 border border-accent-yellow/30' : 'bg-navy-800 border border-navy-700'}`}>
        {isStale && <span className="px-2 py-0.5 bg-accent-yellow/20 text-accent-yellow rounded text-xs font-medium">Warning</span>}
        <span className={isStale ? 'text-accent-yellow' : 'text-slate-400'}>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-xl bg-navy-800 border border-navy-700 p-4 flex flex-col items-center justify-center">
        ...
        <div className="col-span-2 rounded-xl bg-navy-800 border border-navy-700 p-4 overflow-x-auto">
          <p className="text-sm text-slate-400 mb-3">Open Tickets</p>
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.openTickets.map((ticket) => (
                <tr key={ticket.id} className="border-b border-navy-700/50">
                  <td className="py-2 pr-3 text-electric-400 font-mono text-xs">{ticket.id}</td>
                  <td className="py-2 pr-3 text-slate-300 max-w-xs truncate">{ticket.title}</td>
                  <td className="py-2 pr-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${ticket.status === 'In Progress' ? 'bg-electric-500/15 text-electric-400' : 'bg-accent-yellow/15 text-accent-yellow'}`}>
                      {ticket.status}
                    </span>
                  </td>
                  <td className="py-2 text-slate-400 text-xs">{timeAgo(ticket.openSince)}</td>
                </tr>
              ))}
            </tbody>
          </table>
